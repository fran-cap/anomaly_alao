...
| Repeated `time_global()` (2+) | `local tg = time_global()` | Medium - `Device.dwTimeGlobal` is written once per frame, so every read inside one body returns the same number. Threshold 2, not 4: each read is an engine C call that also aborts the LuaJIT trace. Skipped when the body has a `while`/`repeat` or a `coroutine.yield`, when it times itself (`local t0 = time_global()` ... `time_global() - t0`), when no read runs unconditionally (so the hoist would add a call on some path), and when a hoisted declaration would land above an early `return` |
...
| `once_latch_local_to_function` | A "do once" latch declared `local` **inside** a function body: `local done = false` ... `if not done then <work> done = true end`. The flag is fresh on every call, so the guard is always taken and the work the author meant to run once runs on every call - every frame when the body is a per-frame callback. Never auto-fixed: hoisting the flag to module level changes what the script does. Deliberately narrow (one guard, the flip at the tail of it, no loop in between, no closure sees the flag), because a wrong RED wastes a modder's time |
