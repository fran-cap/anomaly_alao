"""Build the I-065 variant overlay: agent-I063-b plus the squad stagger mod's script.

The baseline arm of the I-065 comparison is `agent-I063-b` itself (full ALAO +
the four gen-4 patches + I-057 + alao-prewarm v1.3), so the variant is that tree
with exactly one file added:

    gamedata/scripts/zzz_alao_squad_stagger.script    lab/mods/alao-squad-stagger
...
copied tree, because an overlay that does not compile wastes a run.

    py -3.12 lab/coord/i065_build_overlay.py [--out <dir>]
"""
...
OUT = Path(r"C:\code\GIT\anomaly_alao\lab\coord\overlays\agent-I065-b")
MOD = REPO / "lab" / "mods" / "alao-squad-stagger"
ADDED = ("zzz_alao_squad_stagger.script",)
...
def main(argv=None) -> int:
    ...
    if not a.base.is_dir():
        ...
        return 2
    if a.out.resolve() == a.base.resolve():
        print("REFUSING: --out is the baseline overlay", file=sys.stderr)
        return 2
    ...
    # the arms must differ by the added files and nothing else
    ours = {f"gamedata/scripts/{n}" for n in ADDED} | {"i065_manifest.json", "meta.ini"}
    theirs = {"meta.ini"}
    base_d = tree_digest(a.base, theirs)
    out_d = tree_digest(a.out, ours)
    if base_d != out_d:
        diff = sorted(set(base_d.items()) ^ set(out_d.items()))
        print(f"REFUSING: variant differs from the baseline beyond the added files: {diff[:10]}",
              file=sys.stderr)
        return 5
    ...
    report = {
        ...
        "scripts_total": n,
        "all_compile": True,
        "identical_to_base_otherwise": True,
    }
    (a.out / "i065_manifest.json").write_text(json.dumps(report, indent=2))
    (a.out / "meta.ini").write_text(
        "[General]\ncategory=\ncomments=I-065 arm: agent-I063-b plus "
        + " + ".join(ADDED) + " (lab/mods/alao-squad-stagger). Safe to delete.\n",
        encoding="utf-8")
    print(f"{a.out}: {n} scripts, {len(added)} added, 0 replaced, "
          "all compile under LuaJIT 2.0, otherwise identical to the baseline")
    return 0
