...
NOT RUN CLEANLY YET. The one run taken during the I-050a session is void because
agent-I052 held the corpus lock at the end of it. (Its 5.6x self-check was
actually fine: jit.off() is the global form, see Bench.__init__. The organizer
put that back at integration.) Check `coord.py board` before AND
...
class Bench:
    def __init__(self, script: Path, jit_on: bool):
        ...
        if not jit_on:
            # jit.off() with no arguments IS the global switch. jit.off(true,true)
            # only covers the calling chunk (the one-liner itself), so it does
            # nothing here - measured under lupa on the scalar self-check loop:
            # on 1.50 ms, jit.off() 6.75 ms, jit.off(true,true) 1.52 ms. The
            # honest interpreted ratio for that loop is ~4.5-5.6x, not ~20x
            # (that figure belongs to microbench.py's heavier self-check).
...
def main(argv=None):
    ...
    print(f"self-check: JIT-off / JIT-on on a scalar loop = {ratio:.1f}x "
          f"({'OK' if ratio > 3 else 'SUSPECT - jit.off may not have taken'})")
