...
| I-040 `time_global()` cache | **kept as GREEN** | Correct and free (never adds a call on any path), 134 sites, 1.24x (2 reads) to 1.88x (7) interpreted with a Lua stub, 2.46x with a C-function stub; 0 of 97 GAMMA sites compile, so there is no JIT-on column. But only 15 live sites are per-frame and 7 of those are UI panels, so the standing-still save saves ~10-14 engine calls per frame = ~0.4 us of 4770 us. In-game delta `20260911-164718-I-040-ec27c2` read +0.09% avg / +2.0% 1% low (Welch t 0.04, 95% CI -13.7 to +14.0 fps): **null**, and usable only as a null because the variant arm was biased in its favour: auditing its own output the agent found a second **pre-existing bug** in the whole caching family: `_collect_function_locals` never walked enclosing scopes, so an inserted `local tg = time_global()` captured a module-level `local tg = 0` and `update_artefacts()` in `drx_da_main_artefacts_movement.script` returned on its first line for ever (at `bae4b0c` the same hole turns a function that returns a module-level `local actor = "..."` into one returning the cached `db.actor`). Fixed in 18756a9 (ancestor-scope locals join the taken-name set, renames to `tg_alao` / `actor_alao`). Score 8.0 -> 6.0. |
| I-012 `x^0.5` -> `math.sqrt` | **kept as GREEN** | Premise half wrong: zero `math.pow(x,0.5)` sites in either corpus; the 25 counted were `math.pow(x,2)`. The real population is bare `x^0.5` (10 GAMMA + 1 vanilla), which ALAO did not touch. 3x interpreted at every K, 1.00x compiled because LuaJIT already folds `^0.5` to a hardware sqrt, so a hot trace already returns sqrt's bits and the rewrite makes the interpreter agree with the JIT. At `-0` / `-inf` bases the rewrite is a genuine behaviour change (`+0` -> `-0`, `+inf` -> `nan`) and only literal negatives are declined; GREEN because all 11 sites take squared distances or luminance, where a negative infinity is already a bug. No FPS run: the one live per-frame site saves 24 ns per frame. Score 7.2 -> 6.0. |
